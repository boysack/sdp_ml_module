...
def parse_arguments():
    ...
    parser.add_argument('--dataset_path', default="data/linear_interpolated", type=str, help='Dataset path', metavar="")
...
def train(args):
    ...
    dataset = ArolDataset(args.dataset_path+"_part02.csv")
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False)
    trainer.fit(model, dataloader)

    dataset = ArolDataset(args.dataset_path+"_part03.csv")
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False)
    trainer.fit(model, dataloader)
